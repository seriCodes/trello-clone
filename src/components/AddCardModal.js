...
let inputModalbtn= uuidv1()
    ...
    React.useEffect(() => {
      //change Id using props or state
        ...
        var inputAddCard = document.getElementById(props.listId);
...
////the window.onclick messes with BoardsModal window.onclick- fix later
        ...
        //     ModalStylerObj.closeOutsideClick(event.target, modal, inputModalbtn,idToggle,props.listId)
        //     console.log('event.target')
        //     console.log(event.target)
    ...
    return (  
        ...
        <Button  id={inputModalbtn}        style={{
          ...
          let input= document.getElementById(props.listId)
        ...
        <input class="add-card-input" id={props.listId} type="text" placeholder="Enter task title..."
        onKeyUp={(event)=>{
